...
- Dragonfly hard floor (ALL Dragonfly instances): the memory **limit** is what Dragonfly reads as its available memory, and v1.38+ requires that available memory to be **strictly greater than `256MiB` per thread** — `256Mi` exactly still fails (`"Found 256.00MiB available... There are 1 threads, so 256.00MiB are required. Exiting..."`). It does not OOM, it refuses to boot and crash-loops the cache. **Always set the Dragonfly memory limit to `512Mi`** (the known-good value; never `256Mi` or lower). The request can stay at `256Mi` to avoid reserving 512Mi on packed nodes — the boot check reads the limit/cgroup-max, not the request. This applies to `argocd-redis`, `authentik-redis`, `immich-redis`, `nextcloud-redis`, and any future Dragonfly CR, regardless of what KRR recommends. CPU follows normal rounding. (Setting these to 100–256Mi caused repeated full ArgoCD + SSO + Immich/Nextcloud cache outages.)
