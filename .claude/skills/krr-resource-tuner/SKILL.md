...
- Dragonfly hard floor (ALL Dragonfly instances): never set any Dragonfly container's memory request or limit below `256Mi`, regardless of what KRR recommends. Dragonfly v1.38+ enforces an architectural minimum of `256MiB` per thread and **exits on startup** if its memory limit is lower (`"There are 1 threads, so 256.00MiB are required. Exiting..."`) — it does not OOM, it refuses to boot, crash-looping the cache. This applies to `argocd-redis`, `authentik-redis`, `immich-redis`, `nextcloud-redis`, and any future Dragonfly CR. If KRR recommends less than `256Mi`, use `256Mi`; if more, round up using the normal memory tiers. CPU follows normal rounding. (Caused a full ArgoCD + SSO + cache outage when these were tuned down to 100–220Mi.)
